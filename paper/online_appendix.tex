...
\begin{table}[htbp]
\caption{Static cost robustness to the economic specification on the Zhao validation sample. P3 uses the common $\alpha=0.95$ target; P3m recomputes the target at the SKU-specific margin-implied critical ratio. Reduction is relative to emp-daily.}
...
Chronos-2's lower static cost is retained throughout the evaluated holding-cost range.
...
These quantities identify a fixed-policy measurement contrast on the Zhao common support: the 98\% bridge is the exact accounting share within that factorial, while the OSA evidence is a separate directional cross-dataset check.
...
The integer support cap is a numerical resolution choice: Zhao uses $v_{\max}=60$ units per day, so an $m$-day convolution has support through $60m$.
